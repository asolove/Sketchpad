...
describe("DisplayTransform", () => {
  test("transform works with different origin and zoom", () => {
    ...
    expect(dt([512, 512])).toMatchObject([1024, 1024]);
    expect(dt([-512, -512])).toMatchObject([0, 0]);

    d.zoom = 2;
    dt = d.displayTransform();
    expect(dt([0, 0])).toMatchObject([512, 512]);
    expect(dt([256, 256])).toMatchObject([1024, 1024]);
    expect(dt([-256, -256])).toMatchObject([0, 0]);
